...
@interface AECollectionViewController ()<UICollectionViewDataSource,UICollectionViewDelegate,UICollectionViewDelegateFlowLayout,AEExamDetailCollectionViewCellDelegate,AEExamPresentViewDelegate>
...
@implementation AECollectionViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    [self.qeustionArr removeAllObjects];
    [self.qeustionArr addObjectsFromArray:[AEUserInfo shareAEUserInfo].collectionArr];
    [self filterArr];
...
- (void)examCleanAllData{
    [[AEUserInfo shareAEUserInfo] removeAllCollection];
    
    [self.qeustionArr removeAllObjects];
    
    [self.collectionView reloadData];
    [self.numButton setTitle:[NSString stringWithFormat:@"%d/%d",0,(int)self.qeustionArr.count] forState:UIControlStateNormal];
}
